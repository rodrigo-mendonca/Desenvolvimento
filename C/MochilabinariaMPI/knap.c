...
char* fname;
int tipo  = 0; //  1 - Sequencial  / 0 roda paralelo
...
int main (int argc, char** argv)
{
    ...
    if (!weight || !profit) {
        fprintf (stderr,"Insufficient memory for %d books with max. capacity %d\n",n_books, bag_cap);
        return ENOMEM;
    }
    init_books ();
    ...
    for (book_i = 0; book_i < n_books; ++book_i)
        if (use_book[book_i]) {
            if (n_sold < 10)
                printf ("\t%d (w %d; p %d)\n",book_i,weight[book_i],profit[book_i]);
            else if (n_sold == 10)
                printf ("\t...\n");
...
void get_opts(int argc, char** argv)
{
    ...
    while ((c = getopt(argc, argv, "h?b:c:f:rxW:P:s:v")) != EOF)
        switch (c) {
            case 'h':
            case '?':
                dump_usage (stdout);
...
void dump_usage(FILE* f)
{
  ...
  fprintf (f, "Usage:"
        "\t\t-h or -?      this message\n"
        "\t\t-b <int>      number of books (%i)\n"
        "\t\t-c <int>      bag capacity (%i)\n"
